...
export async function loadContracts(v) {
    const {ERC20DetailedFactoryStrategy} = await dynamicImport("FactoryStrategies", v, "ERC20Detailed");
    ...
    const {ERC20DetailedWrapper} = await dynamicImport("Wrappers", v, "ERC20Detailed");
    ...
    const ERC20Detailed = new ContractWrappersFactory(
        new ERC20DetailedFactoryStrategy(ERC20DetailedArtifacts, ERC20DetailedWrapper, Connector, v)
    );
