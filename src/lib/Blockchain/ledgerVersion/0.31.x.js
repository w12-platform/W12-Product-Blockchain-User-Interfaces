...
export async function loadContracts(libVersion, realVersion) {
    const {ERC20DetailedFactoryStrategy} = await dynamicImport("FactoryStrategies", libVersion, "ERC20Detailed");
    ...
    const {ERC20DetailedWrapper} = await dynamicImport("Wrappers", libVersion, "ERC20Detailed");
    ...
    const ERC20DetailedArtifacts = await jsonLoader(libVersion, "ERC20Detailed");
    ...
    const ERC20Detailed = new ContractWrappersFactory(
        new ERC20DetailedFactoryStrategy(ERC20DetailedArtifacts, ERC20DetailedWrapper, Connector, libVersion, realVersion)
    );
